...
export const maxMatrix = (current: number, field: Matrix<number>) => field.reduce((_max, row) => Math.max(_max, ...row), current)
export const minMatrix = (current: number, field: Matrix<number>) => field.reduce((_min, row) => Math.min(_min, ...row), current)
